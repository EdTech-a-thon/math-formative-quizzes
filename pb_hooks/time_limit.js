...
//
// This is a deliberate second copy of `src/lib/timeLimit.ts`: hooks run inside
// PocketBase and cannot import from the app's `$lib`. The two copies must
// agree, or a legacy quiz would read one way in the teacher's editor and
// another way on the student's screen. `driver.mjs time-limits` runs both over
// the same table of settings and fails if they ever differ, so change the two
// files together.
...
function resolveTimeLimitSeconds(settings) {
...
function timeLimitLabel(seconds) {
...
function stepTimeLimitSeconds(seconds, direction) {
...
module.exports = {
  ...
  resolveTimeLimitSeconds: resolveTimeLimitSeconds,
  formatTimeLimit: formatTimeLimit,
  timeLimitLabel: timeLimitLabel,
  stepTimeLimitSeconds: stepTimeLimitSeconds,
};

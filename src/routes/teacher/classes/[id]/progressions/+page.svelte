...
<section class="workspace-page"><header class="workspace-heading"><div><p class="eyebrow">LEARNING PATHS</p><h1>Progressions</h1><p>Release one attempt when your class is ready. After each attempt, students wait for you to release the next one.</p></div><div class="workspace-heading-actions"><ImportPdfButton classId={String($page.params.id)} onResult={(text, ok) => { message = ok ? text : ""; error = ok ? "" : text; }} /><a class="primary-action" href={`${base}/new`}><Icon name="plus" size={15} /> New progression</a></div></header>

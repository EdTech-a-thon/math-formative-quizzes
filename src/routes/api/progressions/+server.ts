...
export async function POST({ request, cookies }) {
  ...
  const create = await fetch(`${pocketBaseUrl}/api/collections/progressions/records`, {
    method: "POST",
    headers,
    body: JSON.stringify({ class: body.class, name: body.name.trim(), description: String(body.description || "").trim(), passPercentage: Number(body.passPercentage) || 80, oneAtATime: body.oneAtATime === true, showAnswers: body.showAnswers === true, ...appearanceOf(body) }),
  });

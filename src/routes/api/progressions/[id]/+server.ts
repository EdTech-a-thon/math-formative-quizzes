...
export async function PATCH({ request, cookies, params }) {
  ...
  const update = await fetch(`${pocketBaseUrl}/api/collections/progressions/records/${params.id}`, {
    method: "PATCH",
    headers,
    body: JSON.stringify({ name: body.name.trim(), description: String(body.description || "").trim(), passPercentage: Number(body.passPercentage) || 80, oneAtATime: body.oneAtATime === true, ...appearanceOf(body) }),
  });
